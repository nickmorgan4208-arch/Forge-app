...
4. **Surface check** — a pre-scan step rating surface types (skin/textured good; single-color, shiny, dark need markers/prep; transparent risky) plus a printable **marker & scale sheet**: 8 coded tracking markers, a 100 mm scale bar, and a 50 mm verification square that must be ruler-checked before dimensions are trusted.
5. **Guided capture** — rear camera (`facingMode: environment`), a 36-segment orbit ring driven by device orientation that fills in as angles are covered, auto-capture per angle, manual shutter fallback, torch toggle, haptic tick.
6. **O&P presets** — foot/insole, ankle/AFO, transtibial, transradial, spinal/TLSO, free object.
7. **Reconstruction** — demo build generates a preview mesh on-device (see "Real reconstruction" below); captured frames are the input a production backend consumes.
8. **3D viewer** — Three.js, drag-rotate, pinch/wheel zoom, dimensions (L×W×H mm).
9. **Export** — binary **STL** and **OBJ** writers, free and unlimited on every plan.
10. **Scan library** — IndexedDB, patient labels (guidance to avoid full PHI), reopen/re-export/delete.
